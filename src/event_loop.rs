...
impl<S: VhostUserBackend<B>, B: Bitmap + 'static> VringEpollHandler<S, B> {
    ...
    pub fn register_listener(
        ...
        if data < self.vrings.len() as u64 {
...
mod tests {
    ...
    fn test_vring_epoll_handler() {
        ...
        handler
            .register_listener(eventfd.as_raw_fd(), epoll::Events::EPOLLIN, 0)
            .unwrap_err();
        handler
        ...
        // unregister with invalid `data`.
        handler
            .unregister_listener(eventfd.as_raw_fd(), epoll::Events::EPOLLIN, 0)
            .unwrap_err();
        handler
